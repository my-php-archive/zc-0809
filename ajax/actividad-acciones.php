...
global $db;
	...
	while($p = $db->fetch_array($Execute)){
		$html .= "<div class=\"sep\">
			<span class=\"icon-noti ".filtro_tipos($p['tipo'])."\"></span>
				".urldecode($p['html'])."
				<span class=\"time\" ts=\"".$p['fecha']."\">".TiempoTranscurrido($p['fecha'])."</span>
			
<span class=\"remove\"><a href=\"\" onclick=\"borrar_act('".$p['ida']."',".$p['ida'].", this); return false;\">x</a></span>
...
if($nada == true){
	echo "<div id=\"last-activity-date-week\" class=\"date-sep\"><h3><center>No hay actividad</center></h3></div>";
}
