...
var cuenta = {
	...
	chgpais: function(){
		...
		if(empty(pais)){
			...
			$.ajax({
				...
				data: 'type=provinces&country=' + pais,
				success: function(h){
					switch(h.charAt(0)){
						case '0': //Error
						...
						case '1': //OK
							...
							$(el_provincia).append(h.substring(1)).removeAttr('disabled').val('').focus();
							break;
						case '2': //El pais no tiene provincias
							...
							$('form[name=editarcuenta] input[name=ciudad]').setOptions({
								extraParams: {'type':'cities', 'country': pais}
							}).flushCache();
	...
	chgprovincia: function(nofocus){
		...
		var el_provincia = $('form[name=editarcuenta] select[name=provincia]');
		var provincia = $(el_provincia).val();
		...
		if(provincia == ''){
			...
			$.ajax({
				type: 'GET',
				url: '/registro-geo.php',
				data: 'type=hasCities&province=' + $(el_provincia).val(),
				success: function(h){
					switch(h.charAt(0)){
						case '0': //Error
						...
						case '1': //OK
							...
							//Autocomplete para las ciudades
							$('form[name=editarcuenta] input[name=ciudad]').setOptions({
								extraParams: {'type':'cities', 'province':$(el_provincia).val()}
							}).flushCache();
						...
						case '2': //El pais no tiene provincias
							...
							$('form[name=editarcuenta] input[name=ciudad]').addClass('disabled').attr('disabled', 'disabled');
							if (!nofocus) $('form[name=editarcuenta] input[name=ciudad]').val('').focus();
							break;
					}
				},
	...
	save: function (secc, next) {
		...
		$.ajax({
			type: 'post', url: '/cuenta/cuenta.php', data: params.join('&')+gget('key'), dataType: 'json',
			success: function (r) {
				if (r.error) {
					...
					cuenta.alert(secc, 'Los cambios fueron aceptados y serÃ¡n aplicados en los prÃ³ximos minutos.');
	...
	imagen: {
		...
		add: function (obj) {
			...
			$.ajax({
				type: 'post', url: '/cuenta/cuenta.php', data: 'ajax=1&save=7&action=add&url='+$(url).val()+'&caption='+$(caption).val()+gget('key'), dataType: 'json',
				success: function (r) {
					if (r.error) {
		...
		del: function (obj, id) {
			...
			$.ajax({
				type: 'post', url: '/cuenta/cuenta.php', data: 'ajax=1&save=7&action=del&id='+id+'&'+gget('key'), dataType: 'json',
...
var avatar = {
	...
	upload: function (obj) {
		if ($(obj).hasClass("local")) {
			if ($('input#file-avatar').val()) {
				...
				$.ajaxFileUpload({ url: 'http://neotexz.net/upload.php', fileElementId: 'file-avatar', dataType: 'json', success: this.uploadsuccess });
			}
	...
	crop: function (img) {
		...
		mydialog.body('<img class="avatar-crop" src="http://neotexz.net/Avatares/'+img+'?'+Math.random()+'" onload="mydialog.center()">');
		mydialog.buttons(true, true, 'Guardar', 'avatar.save()', true, false, true, 'Cancelar', 'avatar.close()', true, true);
		$('img.avatar-big').attr('src', 'http://neotexz.net/Avatares/'+img+'?'+Math.random()).bind('load', function(){ $('img.avatar-crop').Jcrop({ aspectRatio: 1, sideHandles: false, setSelect: [ 0, 0, 50, 50 ], onChange: avatar.preview, onSelect: function(c) { avatar.crop_coord = c; } }) });
	},
	reload: function () {
	...
	close: function () {
		...
		if (avatar.success) {
			...
			$.ajax({
				url: '/cuenta/cuenta.php', data: 'ajax=1&save=10&s='+avatar.server+gget('key'), type: 'post',
				success: function(r){
					avatar.current = r;
					avatar.reload();
				}
	...
	save: function () {
		...
		else $.getJSON('http://neotexz.net/upload.php?save=true&id='+this.uid+'&s='+this.server+'&crc='+this.crc+'&coords='+this.crop_coord.x+','+this.crop_coord.y+','+this.crop_coord.x2+','+this.crop_coord.y2+'&callback=?');
	},
