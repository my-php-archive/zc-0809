...
/* Eliminar Comentario Foto */
function borrar_comf(comid, autor){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/comentariof-borrar.php',
		data: 'comid=' + comid + '&autor=' + autor + gget('postid') + gget('key'),
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
		...
		error: function(){
			mydialog.error_500("borrar_comf('"+comid+"')");
		}
	});
}
...
function borrar_comm(comid, autor){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/comentariom-borrar.php',
		data: 'comid=' + comid + '&autor=' + autor + gget('postid') + gget('key'),
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
		...
		error: function(){
			mydialog.error_500("borrar_comm('"+comid+"')");
		}
	});
}
...
/* Votar Fotos */
var votar_foto_votado = false;
function show_votar_foto(force_hide){
	if(votar_foto_votado)
		return;
	if(!force_hide && $('.post-metadata .dar_puntos').css('display') == 'none')
		$('.post-metadata .dar_puntos').show();
	else
...
function votar_foto(puntos, x){
	if(votar_foto_votado)
		return;
	votar_foto_votado = true;
	$.ajax({
		type: 'POST',
		url: '/votarfoto.php',
		data: 'puntos=' + puntos + '&x=' + x + gget('postid'),
		success: function(h){
			show_votar_foto(true);
			$('.dar-puntos').slideUp();
			switch(h.charAt(0)){
				case '0': //Error
					$('.post-metadata .mensajes').addClass('error').html(h.substring(3)).slideDown();
		...
		error: function(){
			votar_foto_votado = false;
			mydialog.error_500("votar_foto('"+puntos+"', '"+x+"')");
		}
	});
}
...
var add_favoritosf_agregado = false;
function add_favoritosf(){
	if(add_favoritosf_agregado)
		return;
	if(!gget('key')){
		mydialog.alert('Login', 'Tenes que estar logueado para realizar esta operaci&oacute;n');
		return;
	}
	add_favoritosf_agregado = true;
	$.ajax({
		type: 'POST',
		url: '/favoritosf-agregar.php',
		data: gget('postid', true) + gget('key'),
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
		...
		error: function(){
			add_favoritosf_agregado = false;
			mydialog.error_500("add_favoritosf()");
		}
	});
}
...
var add_fotoc_enviado = false;
function add_fotoc(mostrar_resp, comentarionum){
	if(add_fotoc_enviado)
		return;
	var textarea = $('#body_comm');
	var text = textarea.val();
	...
	$('.miComentario #gif_cargando').show();
	add_fotoc_enviado = true;
	$.ajax({
		type: 'POST',
		url: '/comentario-foto.php',
		data: 'comentario=' + encodeURIComponent(text) + '&lastid=' + lastid_comm + gget('postid') + gget('key') + '&mostrar_resp=' + mostrar_resp,
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
		...
		error: function(){
			add_fotoc_enviado = false;
			mydialog.error_500("add_fotoc('"+mostrar_resp+"')");
		}
	});
}
...
var add_murog_enviado = false;
function add_murog(mostrar_resp, comentarionum){
	if(add_murog_enviado)
		return;
	var textarea = $('#body_comm');
	var text = textarea.val();
	...
	$('.miComentario #gif_cargando').show();
	add_murog_enviado = true;
	$.ajax({
		type: 'POST',
		url: '/comentario-muro.php',
		data: 'comentario=' + encodeURIComponent(text) + '&lastid=' + lastid_comm + gget('postid') + gget('key') + '&mostrar_resp=' + mostrar_resp,
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
		...
		error: function(){
			add_murog_enviado = false;
			mydialog.error_500("add_murog('"+mostrar_resp+"')");
		}
	});
}
...
function home_search(){
	...
	if(document.getElementById('c_search_engine') && document.getElementById('c_search_engine').checked)
		window.location = 'http://buscador.zincomienzo.net/posts?engine=google&q='+q;
	else if(/poringa/.test(document.domain)) //Esta en Poringa!
		window.location = 'http://www.poringa.net/posts/buscador/zincomienzo/?q='+q;
	else
		window.location = 'http://buscador.zincomienzo.net/posts?q='+q;
}
/* FIN - Buscador Home */

/* Change Country */
function change_country(prefix){
	var site = /poringa/.test(document.domain) ? 'poringa' : 'Zincomienzo';
...
$(document).ready(function(){
	...
	$('div.new-search > div.bar-options > ul > li > a').bind('click', function(){
		var at = $(this).parent('li').attr('class').split('-')[0], site = /poringa/.test(document.domain) ? 'poringa' : 'Zincomienzo';
		$('div.new-search > div.bar-options > ul > li.selected').removeClass('selected');
		$(this).parent('li').addClass('selected');
		$('div.new-search').attr('class', 'new-search '+at);
		$('form[name=search]').attr('action', 'http://buscador.'+site+'.com/'+at);
	});
...
function search_set(obj, x) { $('div.search-in > a').removeClass('search_active'); $(obj).addClass('search_active'); $('form[name=top_search_box]').attr('action', 'http://buscador.zincomienzo.net/'+x); $('#ibuscadorq').focus(); }
...
var notifica = {
	...
	userMenuPopup: function (obj) {
		...
		if (this.cache[cache_id] == 1) {
			$(list).children('li.followw').hide();
			$(list).children('li.unfolloww').show();
		}
		else {
			$(list).children('li.unfolloww').hide();
			$(list).children('li.followw').show();
		}
	},
...
function filtrar_ultima_actividad(tipo, idu ){
	mydialog.close();
	$.ajax({
        type: 'POST',
        url: '/ajax/actividad-acciones.php',
        data: 'tipo=' + tipo + '&idu=' + idu,
        success: function(h){
            switch(h.charAt(0)){
                case '0': //Error ~.~
                    mydialog.alert('Error', h.substring(3));
        ...
        error: function(){
            mydialog.error_500("filtrar_ultima_actividad('"+tipo+"')");
        }
    });
}
...
function com_votar(id_comentario, voto){
	var total_votos = parseInt($('#votos_total_' + id_comentario).text());
    total_votos = (isNaN(total_votos)) ? 0 : total_votos;
	$.ajax({
		type: 'GET',
		url: global_data.url + '/votomas.php',
		data: 'voto=' + voto + '&id_comentario=' + id_comentario + '&postid=' + gget('postid'),
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
					$('#cvoto_' + id_comentario).css({color:'red'}).html(h.substring(3)).fadeIn("fast");
					break;
				case '1': //OK
					$('#cvoto_' + id_comentario).css({color:'green'}).html(h.substring(3)).fadeIn("fast");
					//
					total_votos = total_votos + voto;
					if(total_votos < 0) $('#votos_total_' + id_comentario).removeClass("color_green").addClass("color_red");
					$('#votos_total_' + id_comentario).text(total_votos)
					//
					break;
			}
		},
		error: function(){
			mydialog.error_500('com_post(' + id_comentario + ', ' + voto + ')');
		}
	});
}
...
function puntuarcomenposi(idcoment){
  $("#votaposi_" + idcoment).slideUp(1);
    $.ajax({
    type: "GET",
    url: '/votarposi.php?idcoment=' + idcoment,

    success: function(msg){
      
      var original = $("#votaposi2_" + idcoment).html(); //Guardamos el div original
      


$("#votaposi2_" + idcoment).html(msg); //Escribimos el mensaje
  $("#positivo_" + idcoment).remove();
$("#votaposi_" + idcoment).fadeIn("slow");
$('#votaposi_' + idcoment).html(parseInt($('#votaposi_'+ idcoment).html()) + 1);
...
function puntuarcomennega(idcoment){
  $("#votaposi_" + idcoment).slideUp(1);
    $.ajax({
    type: "GET",
    url: '/votar2.php?idcoment=' + idcoment,
