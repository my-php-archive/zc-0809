...
$aux = substr( $_SERVER['REQUEST_URI'], strlen('/fotos/'.$categoria1.'/'));
...
$_pagi_sql = "SELECT id, elim, id_autor, titulo, contenido, privado, coments, foto, categoria FROM fotos where id='$id'";
...
$sql = "SELECT p.*, u.* FROM fotos as p, usuarios as u WHERE p.id_autor=u.id AND p.id='$id'";
...
$sql = "SELECT u.id, u.rango, p.elim, p.id_autor, p.titulo, p.contenido, p.categoria, p.privado, p.coments, p.foto, p.sticky, p.patrocinado ";
$sql.= "FROM (fotos as p, usuarios as u) WHERE p.id='$id'";
$rs = mysql_query($sql, $con);
...
$sql = "SELECT p.id, p.titulo, p.id_autor, p.categoria, c.id_categoria, c.link_categoria, c.nom_categoria, u.nick, u.id FROM (fotos as p, categorias as c, usuarios as u) WHERE p.id='$id' AND p.categoria=c.id_categoria AND u.id=p.id_autor";
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
$dtitulo =$row['titulo'];
$nick	 =$row['nick'];
...
$Post_Rel = "SELECT p.titulo, p.puntos, p.categoria, p.id, p.fecha, c.id_categoria, c.nom_categoria, c.link_categoria 
			 FROM (fotos AS p, categorias AS c) WHERE p.categoria=c.id_categoria AND p.elim=0 
...
?>
	<li class="categoriaPost <?=$post['link_categoria']?>">
				<a rel="dc:relation" href="/fotos/<?=$post['link_categoria']?>/<?=$post['id']?>/<?=corregir($post['titulo'])?>.html" title="<?=$post['titulo']?>"><?=$post['titulo']?></a>
	</li>
...
if ($elim==0 and $esta==1 or $rangoz['rango']==255 or $rangoz['rango']==100 or $rangoz['rango']==655 or $rangoz['rango']==755)
{

  if($denuncias<2 or $rangoz['rango']==255 or $rangoz['rango']==100 or $rangoz['rango']==655 or $rangoz['rango']==755)
  {
...
?>
   <!-- Perfil logeado-->
	<div class="post-autor vcard">
		<div class="box_title">
			<div class="box_txt post_autor">Foto De:</div>
			<div class="box_rrs">
				<div class="box_rss">
					<a href="/rss/posts-usuario/<?=$id_a?>">
						<span style="position:relative;"><img border=0 src="<?=$images?>/big1v11.png" style="position:absolute; top:-354px; clip:rect(352px 16px 368px 0px);" alt="RSS con fotos de <?=$row['nick']?>" title="RSS con fotos de <?=$row['nick']?>" /><img border=0 src="<?=$images?>/space.gif" style="width:14px;height:12px" /></span>					</a>
				</div>
...
?><span style="position:relative;"><img border=0 src="<?=$images?>/big2v1.png" style="<?=$rng2['img_rango']?>" alt="<?=$rng2['nom_rango']?>" title="<?=$rng2['nom_rango']?>" /><img border=0 src="<?=$images?>/space.gif" style="width:17px;height:16px" align="absmiddle" /><?=imgsx(sx($row['sexo']))?>&nbsp;<img title="<?=$pis2['nombre_pais']?>" src="<?=$images?>/flags/<?=bandera($row['pais'])?>.png" width="16" height="11" align="absmiddle" alt="<?=($row['nombre_pais'])?>" hspace="3"> <?php if ($id_user!=$id_autor){ ?><a href="/mensajes/a/<?=$row['nick']?>"><img align="absmiddle" src="<?=$images?>/icon-mensajes-recibidos.gif" title="Enviar mensaje privado" alt="Enviar mensaje privado" /></a>
			<hr class="divider"/>
<?php
...
<!-- Cuerpo -->
	<div class="post-contenedor">
		<div class="post-title">
			...
			<h1 property="dc:title"><?=htmlentities($titulo2)?></h1>
...
?>			
			...
			<center><img class="imagen" src="<?=no_i($foto)?>" border="0"></center>
<br><?=no_i(BBparse($contenido))?>
			</span>
...
if($_SESSION['user']!=null){
$sql = "SELECT p.coments, u.rango, u.puntosdar, u.id, k.puntosdar AS Kpuntosdar, k.id, k.rango AS KRango, p.id_autor, u.nick as nick_autor, p.seguidores, p.categoria, p.fecha, p.puntos AS PPuntos, p.comentarios, p.visitas, p.foto, c.nom_categoria, c.link_categoria, c.id_categoria 
		FROM (fotos as p, usuarios as u, usuarios as k)
...
$sql = "SELECT p.coments, u.rango AS Urango, u.puntosdar, u.id, k.puntosdar AS Kpuntosdar, k.id, k.rango AS KRango, p.id_autor, p.seguidores, u.nick as nick_autor, p.categoria, p.fecha, p.puntos AS PPuntos, p.comentarios, p.visitas, p.foto, c.nom_categoria, c.link_categoria, c.id_categoria 
		FROM (fotos as p, usuarios as u, usuarios as k)
		inner join categorias as c
		on c.id_categoria = p.categoria
 		where p.id='$id'
...
$query2	=	mysql_query("SELECT * FROM favoritosfotos WHERE id_post = '$id'");
...
<li><a class="btn_g" href="javascript:add_favoritosf()"><span class="icons agregar_favoritos">Agregar a Favoritos</span></a></li>
...
<li><a class="btn_g denuncia_form" href="javascript:;"><span class="icons denunciar_post">Denunciar</span></a></li>
			...
			<ul class="post-cat-date">
				...
				<li><strong>Creado:</strong> <span property="dc:date" content="<?php echo date("Y-m-d H:m:s", date($row['fecha'])); ?>"><?=$row['fecha']?></span></li>
			</ul>
			<div class="clearfix"></div>
	...
	<div class="floatR" style="width: 766px; wi\dth: 765px">
		<div class="post-relacionados">
			<h4>Otras fotos que te van a interesar:</h4>
			<ul>
<?php
	$busqueda = mostrartags($row['foto']);
	$trozos = explode(", ", $busqueda);
	$numero = count($trozos);
	
if($numero==0){
echo'<br>Este post no tiene foto';
}else{
$Post_Relacionados	=	"SELECT DISTINCT *
                        FROM fotos, categorias
						WHERE MATCH fotos.foto
						AGAINST ('$busqueda')
						AND fotos.categoria=categorias.id_categoria AND fotos.elim = 0
						ORDER BY fotos.foto DESC
						LIMIT 0, 10";
...
<li class="categoriaPost <?=$PR['link_categoria']?>">
	<a rel="dc:relation" href="/fotos/<?=$PR['link_categoria']?>/<?=$PR['id']?>/<?=corregir($PR['titulo'])?>.html" title="<?=$PR['titulo']?>"><?=$PR['titulo']?></a>
</li>
<?php
}}
?>
...
<li><a class="btn_g" href="/imprimir-post/<?php echo''.$id.''; ?>"><img src="/images/imprimir.ico" width="14" height="14" />&nbsp;&nbsp;Imprimir</a></li>
			...
			<div class="tags-block">
				...
				<?php
if($row['foto']!=null){
$foto	=	mostrartags($row['foto']);
$myfoto	=	explode(", ", $foto);
$cant_foto = count($myfoto);
foreach ($myfoto as $tag){
echo'<a href="/foto/'.$tag.'" rel="tag">'.$tag.'</a>';
$cfoto++;
if ($cfoto<=$cant_foto-1){
echo " - ";
}}}else{echo'El post no tiene foto';}
?>
							</div>

			<ul class="post-cat-date">
				<li><strong>Categor&iacute;a:</strong> <a href="/fotos/<?=$row['link_categoria']?>/"><?=$row['nom_categoria']?></a></li>
...
if ($id_user==$id_autor){
echo'';}
echo'      		<div class="comentarios-title">
...
$limit_posts=500;
$request = mysql_query("SELECT * FROM comentariosfotos where id_post='$id'");
...
$num=0;
$sqlcom = mysql_query("SELECT * FROM comentariosfotos WHERE id_post='$id' order by id ASC LIMIT $RegistrosAEmpezar, $limit_posts");
...
echo '<div class="paginadorCom">
      <div class="before floatL">
      		<a href="" onclick="return false" class="desactivado"><b>&laquo; Anterior</b></a>
      </div>
      <div style="float:left;width: 530px">
       ...
       for ($numbers = 1; $numbers<=$PagUlt; $numbers++){
           ...
           echo'<li class="numbers"><a href="'.$_SERVER['REQUEST_URI'].'#comentarios">'.$numbers.'</a></li>';}}
echo' </ul>
      </div>
      
      <div class="floatR next">
                	<a href="'.$_SERVER['REQUEST_URI'].'#comentarios"><b>Siguiente &raquo;</b></a>
      </div>
...
if($NroRegistros<=0 and $row['coments']!='on'){echo'
		<div class="clearfix"></div>
			...
			echo'<div class="clearfix"></div>
				<div style="font-weight: bold; font-size: 14px;text-align: center;color: #666;margin: 20px 0 20px 0;">La foto se encuentra cerrado y no se permiten comentarios.</div>';}
		...
		<div class="comment-box">
			...
			<div class="comment-info clearbeta">
				...
				<div class="floatR answerOptions">
					...
					if($rangoz['rango']==255 or $rangoz['rango']==100 or $rangoz['rango']==655 or $rangoz['rango']==755 or $key == $comenz['id_autor'] or $key == $id_autor){
					echo '<li class="iconDelete"><a href="javascript:borrar_comf('.$comenz['id'].')"><span class="borrar-comentario"></span></a></li>';}
			        echo '
					</ul>
				</div>					
			</div>
			<div class="comment-content">'.BBparse(htmlentities($comenz['comentario'])).'</div>
		</div>
	</div>
</div>';}
...
echo'<div class="comentarios-title">
	      ...
	      <div class="before floatL">
	      							<a href="'.$_SERVER['REQUEST_URI'].'#comentarios-abajo"><b>&laquo; Anterior</b></a>
						      </div>
	      <div style="float:left;width: 530px">
	        <ul>
	        	        	          <li class="numbers"><a class="here" href="'.$_SERVER['REQUEST_URI'].'#comentarios-abajo">1</a></li>
	        	        <li class="numbers"><a href="'.$_SERVER['REQUEST_URI'].'">2</a></li>
	        	        	        </ul>
	      </div>
	      <div class="floatR next">
	        	        	<a href="'.$_SERVER['REQUEST_URI'].'" onclick="return false" class="desactivado"><b>Siguiente &raquo;</b></a>
	        	      </div>
	      <div class="clearBoth"></div>
	    </div>
    </div>';}
...
echo'
	<div class="miComentario">
		...
		<div class="answerInfo">
			<img class="avatar-48" width="48" height="48" src="/images/avatar.gif" alt="" onerror="error_avatar(this, '.$moder2['id'].', 48)" />
		</div>
		...
		<div class="answerTxt">
		  <div class="Container">
				...
				<div class="buttons" style="text-align:left">
					<div class="floatL">
						<input type="button" onclick="add_fotoc(\'true\')" class="mBtn btnOk" value="Enviar Comentario" tabindex="2" />
					</div>
...
<div class="post-deleted post-privado clearbeta">
	<div class="content-splash">
		...
		<h3>Esta Foto es privada, s&oacute;lo los usuarios registrados de '.$comunidad.' pueden acceder.</h3>
...
$Potrel	=	"SELECT DISTINCT *
		     ...
		     WHERE MATCH fotos.foto
			 AGAINST ('$busqueda')
			 AND fotos.categoria=categorias.id_categoria 
			 ORDER BY fotos.foto DESC
			 LIMIT 0, 10";
                        
$fotos    =    mysql_query($Potrel, $con);
while($post=mysql_fetch_array($fotos))
{echo'
<li class="categoriaPost '.$post['link_categoria'].'">
					<a rel="dc:relation" href="/fotos/'.$post['link_categoria'].'/'.$post['id'].'/'.corregir($post['titulo']).'.html" title="'.$post['titulo'].'">'.$post['titulo'].'</a>
				</li>';
