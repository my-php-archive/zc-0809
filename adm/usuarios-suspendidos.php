...
<div class='boxy xtralarge2'>
			...
			?>
			<br />
<div id="error_data" class="emptyData" style="display: block; "><center><b>S&oacute;lo puedes quitar la suspenci&oacute;n a los usuarios que t&uacute; hayas suspendido</b></center></div>
			
			<?php
			}elseif($rangoz['rango'] == "255" or $rangoz['rango'] == "655" or $rangoz['rango'] == "755"){
			echo'<br /><div class="emptyData">Podr&aacute;s seleccionar a los moderadores</div><hr>';
			   echo'
			   <center><form action="usuarios-suspendidos.php" method="get" align= "center">
			   <select name="modera">';
			   $sqlM = mysql_query('
			   SELECT * 
               FROM usuarios
			   ...
			   OR rango = "655"
			   ');
			   while($mod = mysql_fetch_assoc($sqlM)){
			      echo'<option value="'.$mod['nick'].'">'.$mod['nick'].'</option>';
			   }
			   echo'</select></center>
<br>
			     <center><input type="submit" class="mBtn btnOk" style="display:inline-block" value="Filtrar"></center>
				 </form><hr>
			   ';
			}
			?>
	...
	<tbody>
		<tbody>
<?php
	$_pagi_sql = "SELECT * FROM suspendidos where activo='1' ";
	if($rangoz['rango'] == "100"){
	   $_pagi_sql.="AND modera = '".$_SESSION['user']."'";
	}
	if($_GET['modera']){
	   $_pagi_sql.="AND modera = '".$_GET['modera']."'";
	}
...
?>
	<tr id="div_<?=$nick?>">
		...
		<td><?=$fecha1?><br />Termina: <?=$fecha2?> Dia/s</td>
		<td>
		<a id="change_status" href="javascript:desbanear_usuario('<?=$nick?>')"><img src="/images/reactivar.png" alt="Desbanear" title="Reactivar usuario" /></a>
		</td>
   </tr>
<?php
}
...
?>
	...
	if(!mysql_num_rows($rs)) {
	   echo'<br><div class="emptyData">No has suspendido a ning&uacute;n usuario</div>';
	}
	?>
		<center><?php echo "".$_pagi_navegacion."";?></center>
<br/>
<center>
<input onclick="javascript:com.banear_usuario()" title="Suspender Usuario" value="Suspender Usuario" class="mBtn btnDelete" type="button">
</center>
	</div>
		</div>
