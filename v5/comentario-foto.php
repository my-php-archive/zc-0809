...
$mostrar_resp = no_injection($_POST['mostrar_resp']);
	...
	$sql = "SELECT id_autor FROM fotos WHERE id='".$postid."'";
...
$q2 = mysql_query("SELECT id_autor FROM fotos WHERE id = '{$postid}'");
...
$sqlp=$db->query("SELECT s.id_post_seguido, s.id_seguidor, u.nick, u.id FROM seguidor as s, usuarios as u WHERE s.id_post_seguido='$postid' AND s.id_seguidor=u.id ORDER BY id desc");
...
mysql_query("UPDATE fotos Set comentarios=comentarios+'1' Where id='".$postid."'");
mysql_query("UPDATE usuarios Set numcomentarios=numcomentarios+'1',ultimaaccion2=unix_timestamp() Where id = '".$key."'");
mysql_query("INSERT INTO comentariosfotos (id_post, id_autor, autor, comentario, fecha) VALUES ('$postid', '$key', '$user', '$comentario', unix_timestamp())");
$idc = mysql_insert_id();
...
$p = mysql_query("SELECT p.id, p.titulo, c.link_categoria, c.id_categoria
                FROM (fotos AS p, categorias AS c)
                WHERE p.id = {$postid} and p.categoria = c.id_categoria");
$p = mysql_fetch_object(($p));
$href2 = "/fotos/{$p->link_categoria}/{$p->id}/".corregir($p->titulo).".html";

$tipo_ac = tipo_accion("comento-post");
mysql_query("insert into acciones (ida,idu,tipo,html,fecha,otro) values (NULL,'{$key}','{$tipo_ac}','Coment&oacute; en la foto <a href=\"$href2\" >".urlencode($p->titulo)."</a>',unix_timestamp(),'')");
