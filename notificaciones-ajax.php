...
$n2= time();
$miembro =	$_SESSION['id'];
if(!$miembro) die("<div class='emptyData'>Para realizar esta accion necesitas autentificarte</div>");
$action = no_injection($_POST['action']);
$type = no_injection($_POST['type']);
$obj = no_injection($_POST['obj']);

if($action==last){
$nots = mysql_query("SELECT n.*, u.nick FROM notificaciones n, usuarios u WHERE u.id=n.id_autor AND n.id_user='$miembro' AND n.estatus = 1 ORDER BY n.fecha DESC LIMIT 20");

if(!mysql_num_rows($nots)){echo'<div class="emptyData">No hay nuevas notificaciones</div>';}
...
# Mas de 1 comentario en tu post by zincomienzo|timbalentimba
/*$comment_mas = mysql_query("SELECT * FROM notificaciones WHERE detalle = 'sprite-balloon-left' AND id_user = '$miembro' AND status = 1");
$rowComment = mysql_fetch_assoc($comment_mas);
   if(mysql_num_rows($comment_mas) >= 1){
    if ($rowComment['detalle']=='sprite-balloon-left'){echo'Mas de uno';}
   }
   else
   {
    if ($rowComment['detalle']=='sprite-balloon-left'){echo'Mas de uno';}
