...
var add_muro_enviado = false;
function add_muro(mostrar_resp, comentarionum){
	if(add_muro_enviado)
		return;
	...
	$.ajax({
		...
		error: function(){
			add_muro_enviado = false;
			mydialog.error_500("add_muro('"+mostrar_resp+"')");
		}
	});
}
...
//Borrar MP
function borrar_mp(id_mensaje){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/admin/droop-mp.php',
		data: 'id='+id_mensaje,
		success: function(h){
			switch(h.charAt(0)){
				case '0':
					...
					break;
				case '1':
					$('#div_'+id_mensaje).fadeOut('normal', function(){ $(this).remove(); });
					break;
			}
		},
		error: function(){
			mydialog.error_500("borrar_mp('"+id_mensaje+"')");
		}
	});
}
...
function borrar_nota(id){
	...
	$.ajax({
		...
		error: function(){
			mydialog.error_500("borrar_nota('"+id+"')");
		}
	});
}
...
//Borrar Muro
function borrar_muro(ida){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/admin/droop-muro.php',
		data: 'id='+ida,
		success: function(h){
			switch(h.charAt(0)){
				case '0':
					...
					break;
				case '1':
					$('#div_cmnt_'+ida).fadeOut('normal', function(){ $(this).remove(); });
					break;
			}
		},
		error: function(){
			mydialog.error_500("borrar_muro('"+ida+"')");
		}
	});
}
...
//Quitar Denuncia Usuarios
function borrar_denunciau(id){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/admin/droop-denunciau.php',
		data: 'id='+id,
		success: function(h){
			switch(h.charAt(0)){
				case '0':
		...
		error: function(){
			mydialog.error_500("borrar_denunciau('"+id+"')");
		}
	});
}
...
//Quitar Denuncia Usuarios
function borrar_medalla(id){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/admin/droop-medallas.php',
		data: 'id='+id,
		success: function(h){
			switch(h.charAt(0)){
				case '0':
		...
		error: function(){
			mydialog.error_500("borrar_medalla('"+id+"')");
		}
	});
}
